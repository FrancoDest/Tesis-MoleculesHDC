...
def clean_dataset(X, Y):
    ...
    for x, y in zip(X, Y):
        # Detect BigSMILES syntax -> skip DeepChem validation
        if any(tag in x for tag in ["{", "}", "[>", "<]"]):
            X_clean.append(x)
            Y_clean.append(y)
            continue
...
def data_tokenize_smiles_pretrained(X, num_tokens=1500):
    ...
    # The original code attempted to open a CSV which can raise when the
    # file is not a valid BPE codes file. Fall back to characterwise
    # tokenization on failure to avoid crashing the whole run.
    try:
        # Try to find a valid BPE codes file
        import os.path
        if os.path.isfile('./SmilesPE_codes'):
            spe_vob = codecs.open('./SmilesPE_codes')
        elif os.path.isfile('./data/SmilesPE_codes'):
            spe_vob = codecs.open('./data/SmilesPE_codes')
        else:
            # No BPE codes file found, fall back to characterwise
            print("Warning: SmilesPE BPE codes file not found. Falling back to characterwise tokenization.")
            return data_tokenize_characterwise(X, num_tokens=num_tokens)
        
        spe = SPE_Tokenizer(spe_vob)
    except Exception as e:
        print(f"Warning: could not load SMILES-PE tokenizer (error: {e}). Falling back to characterwise tokenization.")
        return data_tokenize_characterwise(X, num_tokens=num_tokens)
...
def create_data_HV(data_tokenized, gramsize=1, num_tokens=1500, dim=10000, max_pos=256, random_state=0):
    ...
    # `random.seed` siembra el RNG de Python, pero la memoria de items sale de
    # numpy: quedaba efectivamente sin sembrar. Y como cada iteracion corre en
    # su propio proceso (ProcessPoolExecutor en MoleHD.py), el estado global de
    # numpy que hereda cada worker no es reproducible entre corridas. Un
    # Generator local es preferible a np.random.seed justo por eso: no depende
    # del estado global del proceso padre ni lo pisa para el resto del worker.
    rng = np.random.default_rng(random_state)
    item_mem = rng.choice((-1, 1), (num_tokens+1, dim))
...
def train_test_split_scaffold(X, Y, data_HV, test_size=0.20, random_state=10):
    ...
    try:
        ...
        raise ImportError(
            "Scaffold split requires deepchem to be installed. "
            "Use --split_type random or random_stratified, or install deepchem support."
        ) from exc
...
def retrain(assoc_mem, X_tr, Y_tr, epochs=150, dim=10000, threshold=1024):
    ...
    for epoch in range(epochs):
        for i in range(len(Y_tr)):
            dist = cdist(X_tr[i].reshape(1, dim), assoc_mem, metric='cosine')[0]
...
def inference(assoc_mem, X_te, Y_te, dim=10000):
    """generates prediction and per-class score on testing dataset

    Generalized to N classes: assoc_mem may have any number of rows (one per
    class), not just 2. Prediction is the class with the smallest cosine
    distance (highest similarity). The per-class score is a cosine
    similarity in [0, 1] (monotonic in the cosine distance, so ROC AUC --
    which only depends on score ranking -- is unaffected for the binary case
    compared to the original (dist[0]-dist[1]+2)/4 formula).

    Args:
        assoc_mem (ndarray): An associative memory of size (n, dim), where n is the number of labels
        X_te (list): testing dataset with hypervector representing molecules
        Y_te (list): the label of all molecules in the testing dataset
        dim (int): dimension of hypervector
    ...
    Returns:
        ...
        Y_scores (ndarray): per-class similarity scores, shape (n_samples, n_classes)
    """

    Y_pred = []
    Y_scores = []
    for i in range(len(Y_te)):
        dist = cdist(X_te[i].reshape(1, dim), assoc_mem, metric='cosine')[0]
        pred = int(np.argmin(dist))
        Y_pred.append(pred)
        Y_scores.append((1.0 - dist / 2.0).tolist())

    return (Y_pred, np.asarray(Y_scores))
