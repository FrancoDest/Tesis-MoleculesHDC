"""Variante de hdc_pipeline/run_hdc_pipeline.py.

Flujo (mas corto que el de HDC porque no hay JL ni conversion a
hypervectors):
1. leer CSV base
2. generar embeddings Mole-BERT una unica vez para todas las moleculas
3. separar train/test 80/20 (repetido N iteraciones)
4. entrenar un RandomForestClassifier directo sobre los embeddings de train
5. evaluar sobre los embeddings de test
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ejecuta el flujo CSV -> embeddings Mole-BERT -> train/test split -> "
            "RandomForestClassifier -> evaluacion (sin JL ni HDC)."
        )
    )
    parser.add_argument(
        "--input-csv",
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Nucleos a usar para correr las iteraciones del Random Forest en paralelo. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "como feature numerica extra al embedding antes de entrenar el Random Forest."
        ),
    )
...
def build_output_file_map(input_csv: str, output_dir: str) -> dict[str, str]:
    ...
    return {
        ...
        "metrics": str(root / f"{base_name}_rf_metrics.json"),
        "predictions": str(root / f"{base_name}_rf_test_predictions.csv"),
    }
...
def _run_single_iteration(task: tuple[int, int, float, int, bool]) -> dict[str, object]:
    """Corre un split 80/20 + RandomForestClassifier completo.
    ...
    """
    iteration, iteration_seed, test_size, n_estimators, keep_artifacts = task
...
def main() -> None:
    ...
    print(f"Usando {n_jobs} nucleos para correr {args.iterations} iteraciones en paralelo (Random Forest).")

    tasks = [
        (iteration, args.random_state + iteration, args.test_size, args.n_estimators, iteration == 0)
