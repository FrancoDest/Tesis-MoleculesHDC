"""Capa compacta del pipeline Random Forest montado arriba de Mole-BERT.

Variante de hdc_pipeline/core.py: reusa exactamente la misma generacion de
embeddings Mole-BERT (mismo encoder GIN, mismo pooling), pero se salta la
proyeccion Johnson-Lindenstrauss y la conversion a hypervectors bipolares
-- esos dos pasos existen para que el clasificador final (lineal, SGD) tenga
un espacio de mayor dimension donde separar mejor las clases, algo que un
Random Forest no necesita: puede entrenar directo sobre el embedding de 300
dimensiones que ya produce el encoder.
"""
