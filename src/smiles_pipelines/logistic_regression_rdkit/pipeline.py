...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Entrena una Regresion Logistica para datasets binarios tipo BBBP usando "
            "descriptores de RDKit y evalua con split 80/20."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Cantidad maxima de iteraciones del solver (lbfgs) hasta converger.",
    )
    parser.add_argument(
        "--c-reg",
        type=float,
        default=1.0,
        help="Inverso de la fuerza de regularizacion L2 (parametro C de sklearn).",
    )
    parser.add_argument(
        "--iterations",
        type=int,
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Nucleos a usar para calcular descriptores RDKit. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Si se pasa junto con --frac-b-column, se suma como feature numerica extra "
            "a los descriptores RDKit."
        ),
    )
...
def build_feature_table(
    dataframe: pd.DataFrame, smiles_column: str, n_jobs: int = 1
) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, int]]:
    row_indices = list(dataframe.index)
    smiles_values = dataframe[smiles_column].tolist()

    if n_jobs > 1 and len(smiles_values) > 1:
        chunksize = max(1, len(smiles_values) // (n_jobs * 4))
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            all_descriptors = list(
                executor.map(smiles_to_descriptor_row, smiles_values, chunksize=chunksize)
            )
    else:
        all_descriptors = [smiles_to_descriptor_row(smiles) for smiles in smiles_values]

    valid_indices = []
    descriptor_rows = []
    for row_index, descriptors in zip(row_indices, all_descriptors):
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, y_proba: np.ndarray) -> dict[str, float | list[list[int]]]:
    """y_proba: matriz completa de probabilidades por clase (n_samples, n_classes).
    Si el target tiene mas de 2 clases , se usa
    promedio macro y ROC AUC one-vs-rest en vez del esquema binario simple."""
...
def build_feature_coefficients(feature_columns, coefficients) -> pd.DataFrame:
    """coefficients: coef_ del LogisticRegression, shape (1, n_features) en
    binario o (n_classes, n_features) en multiclase .
    Para multiclase se reporta el promedio del valor absoluto entre clases
    (magnitud de impacto), ya que no hay un unico signo interpretable."""
    coefficients = np.asarray(coefficients)
    values = coefficients[0] if coefficients.shape[0] == 1 else np.abs(coefficients).mean(axis=0)
    return (
        pd.DataFrame({"descriptor": feature_columns, "coefficient": values})
...
def build_model(max_iter: int, c_reg: float, random_state: int) -> Pipeline:
    return Pipeline(
        [
...
def run_training_loop(
    ...
    best_feature_coefficients = None
    ...
    for iteration in range(iterations):
        ...
        model = build_model(max_iter=max_iter, c_reg=c_reg, random_state=iteration_seed)
        ...
        if metrics["roc_auc"] > best_auroc:
            ...
            best_feature_coefficients = build_feature_coefficients(
    ...
    return {
        ...
        "best_feature_coefficients": best_feature_coefficients,
...
def save_outputs(
    ...
    best_feature_coefficients: pd.DataFrame,
) -> None:
    ...
    best_feature_coefficients.to_csv(output_dir / "feature_coefficients.csv", index=False)
...
def print_save_summary(output_dir: Path) -> None:
    ...
    print(f"Coeficientes guardados en: {output_dir / 'feature_coefficients.csv'}")
...
def main() -> None:
    ...
    print(f"Usando {n_jobs} nucleos para descriptores RDKit.")
    ...
    if args.frac_a_column and args.frac_b_column:
        frac_matrix = clean_dataframe[[args.frac_a_column, args.frac_b_column]].to_numpy(dtype=float)
        X = np.concatenate([X, frac_matrix], axis=1)
        feature_columns = feature_columns + [args.frac_a_column, args.frac_b_column]
    ...
    summary = {
        "pipeline": "logistic_regression_rdkit",
    ...
    save_outputs(
        ...
        best_feature_coefficients=training_result["best_feature_coefficients"],
    )
