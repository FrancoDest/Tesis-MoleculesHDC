...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Entrena una red neuronal (MLPClassifier) para datasets binarios tipo BBBP "
            "usando descriptores de RDKit y evalua con split 80/20."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Seed para el split y la inicializacion de la red.",
    )
    parser.add_argument(
        "--hidden-layer-sizes",
        default="100",
        help="Tamanos de las capas ocultas separados por coma (ej: '128,64' para dos capas).",
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=0.0001,
        help="Fuerza de regularizacion L2 (parametro alpha de sklearn).",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
        default=500,
        help="Cantidad maxima de epocas del solver (adam) hasta converger o frenar por early stopping.",
    )
    parser.add_argument(
        "--iterations",
        type=int,
...
def parse_hidden_layer_sizes(value: str) -> tuple[int, ...]:
    sizes = tuple(int(piece.strip()) for piece in value.split(",") if piece.strip())
    if not sizes:
        raise ValueError(f"--hidden-layer-sizes invalido: {value!r}")
    return sizes
...
def build_model(hidden_layer_sizes: tuple[int, ...], alpha: float, max_iter: int, random_state: int) -> Pipeline:
    return Pipeline(
        [
        ...
        detailed_rows.append(
            {
                ...
                "n_iter_": int(model.named_steps["classifier"].n_iter_),
...
def main() -> None:
    ...
    print(f"Arquitectura de la red: {hidden_layer_sizes}")
