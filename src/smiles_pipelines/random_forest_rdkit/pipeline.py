...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Nucleos a usar para calcular descriptores RDKit y entrenar el bosque. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "a los descriptores RDKit -- necesaria en datasets donde el mismo "
            "par de moleculas puede tener distinta etiqueta segun la composicion."
        ),
    )
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, y_proba: np.ndarray) -> dict[str, float | list[list[int]]]:
    ...
    Si el target tiene mas de 2 clases , se usa promedio macro y ROC AUC one-vs-rest en vez del
    esquema binario simple."""
...
def main() -> None:
    ...
    print(f"Usando {n_jobs} nucleos para descriptores RDKit y entrenamiento del Random Forest.")
