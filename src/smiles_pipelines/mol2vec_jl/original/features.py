...
def sentences2vec(sentences, model, unseen=None):
    ...
    if hasattr(model.wv, "key_to_index"):
        ...
        get_vector = model.wv.get_vector
    else:  # pragma: no cover - compatibilidad con gensim viejos
        keys = set(model.wv.vocab.keys())
        get_vector = model.wv.word_vec
    vec = []
    if unseen:
        unseen_vec = get_vector(unseen)

    for sentence in sentences:
        if unseen:
            vec.append(sum([get_vector(y) if y in set(sentence) & keys
                       else unseen_vec for y in sentence]))
        else:
            vec.append(sum([get_vector(y) for y in sentence 
